...
// todo whole thing is fucked but shouldn't be used anyways
...
public class PacketFly {
    ...
    private void onMove(CancellablePlayerMoveEvent event) {
        ...
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX() + vel.x, mc.player.getY() + vel.y, mc.player.getZ() + vel.z, mc.player.getYaw(), mc.player.getPitch(), mc.player.isOnGround(), true));

        double y = mc.player.getY() + vel.y;
        y += 1337;
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX() + vel.x, y, mc.player.getZ() + vel.z, mc.player.getYaw(), mc.player.getPitch(), mc.player.isOnGround(), true));
    }
    ...
    private void onPacketRecieve(PacketEvent.Receive event) {
        ...
        //if (EventUtil.getPacket(event) instanceof PlayerPositionLookS2CPacket packet && mc.currentScreen == null) {
        //    mc.getNetworkHandler().sendPacket(new TeleportConfirmC2SPacket(packet.teleportId()));
        //    mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(packet, packet.getY(), packet.getZ(), packet.getYaw(), packet.getPitch(), false));

...
public class MiscUtil {
    ...
    public static boolean shouldEatItem(Item item) {
        if (item == Items.ENCHANTED_GOLDEN_APPLE || item == Items.GOLDEN_APPLE && !Modules.get().get(ElytraBotThreaded.class).allowGaps.get())
            return false;
    ...
    public static void mine(BlockPos blockPos) {
        if (!MeteorClient.mc.player.getMainHandStack().isIn(ItemTags.PICKAXES)) {
    ...
    public static boolean isSolid(BlockPos pos) {
        try {
            // deprecated but don't care if it works (not sure yet)

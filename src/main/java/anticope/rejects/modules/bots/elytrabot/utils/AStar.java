...
import static anticope.rejects.modules.bots.elytrabot.utils.MiscUtil.*;
...
public class AStar {
    ...
    public static ArrayList<BlockPos> addToOpen(BlockPos[] positions, ArrayList<BlockPos> checkPositions, BlockPos current, BlockPos goal, BlockPos start, ArrayList<BlockPos> open, ArrayList<BlockPos> closed) {
        ...
        for (BlockPos pos : positions2) {
            if (!isSolid(pos) && !closed.contains(pos)) {
                // Check if maintainY is enabled and this position is below minYLevel
                if (elytraBot.maintainY.get() && pos.getY() < elytraBot.minYLevel.get()) {
                    // Skip positions below minYLevel unless we're ascending (going up)
                ...
                for (BlockPos check : checkPositions2) {
                    if (Modules.get().get(ElytraBotThreaded.class).botMode.get() == ElytraBotThreaded.PathMode.Highway && !isInRenderDistance(check)) {
                        return null;
                    }
    ...
    public static double fCost(BlockPos pos, BlockPos goal, BlockPos start) {
        ...
        // Apply penalty for positions below minYLevel if maintainY is enabled
        if (elytraBot.maintainY.get() && pos.getY() < elytraBot.minYLevel.get()) {
            // The further below minYLevel, the higher the penalty
            double yDiff = elytraBot.minYLevel.get() - pos.getY();
            fCost += yDiff * 10; // Significant penalty to discourage pathing below minYLevel
        }

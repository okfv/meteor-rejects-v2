...
public class PacketFly extends Module {
    ...
    public void onSendMovementPackets(SendMovementPacketsEvent.Pre event) {
        ...
        boolean movingForward = mc.player.input.playerInput.forward() && !mc.player.input.playerInput.backward();
        boolean movingSideways = mc.player.input.playerInput.left() != mc.player.input.playerInput.right();
        speed = mc.player.input.playerInput.jump() && (checkCollisionBoxes || !(movingForward || movingSideways)) ? (antiKick.get() && !checkCollisionBoxes ? (resetCounter(downDelayFlying.get()) ? -0.032 : verticalSpeed.get()/20) : verticalSpeed.get()/20) : (mc.player.input.playerInput.sneak() ? verticalSpeed.get()/-20 : (!checkCollisionBoxes ? (resetCounter(downDelay.get()) ? (antiKick.get() ? -0.04 : 0.0) : 0.0) : 0.0));

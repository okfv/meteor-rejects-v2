...
//        info("Renaming");
    ...
    private String getFirstItemName(ItemStack stack) {
        ...
        NbtCompound blockEntityTag = compound.getCompound("BlockEntityTag").orElse(null);
        if (blockEntityTag == null) return "";
        NbtList list = blockEntityTag.getList("Items").orElse(null);
        if (list == null || list.isEmpty()) return "";
        var minslot = Byte.MAX_VALUE;
        var name = "";
        for (int i = 0; i < list.size(); i++) {
            var invItemOptional = list.getCompound(i);
            if (invItemOptional.isEmpty()) continue;
            var invItem = invItemOptional.get();
            var invSlot = invItem.getByte("Slot").orElse(Byte.MAX_VALUE);
            if (minslot < invSlot) {
                continue;
            }

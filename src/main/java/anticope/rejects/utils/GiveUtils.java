    ...
    static {
        ENTITY_PRESETS.forEach((preset) -> {
            PRESETS.put(preset.getLeft(), (preview) -> {
                ...
                try {
                    NbtCompound entityTag = StringNbtReader.readCompound(preset.getRight());
                    String entityId = entityTag.getString("id", "minecraft:pig");
                    EntityType<?> entityType = Registries.ENTITY_TYPE.get(Identifier.of(entityId));
                    item.set(DataComponentTypes.ENTITY_DATA, TypedEntityData.create(entityType, entityTag));
        ...
        BLOCK_PRESETS.forEach((preset) -> {
            PRESETS.put(preset.getLeft(), (preview) -> {
                ...
                ItemStack item = preset.getMiddle().getDefaultStack();
                try {
                    NbtCompound blockEntityTag = StringNbtReader.readCompound(preset.getRight());
                    String blockEntityId = blockEntityTag.getString("id", "minecraft:spawner");
                    BlockEntityType<?> blockEntityType = Registries.BLOCK_ENTITY_TYPE.get(Identifier.of(blockEntityId));
                    item.set(DataComponentTypes.BLOCK_ENTITY_DATA, TypedEntityData.create(blockEntityType, blockEntityTag));
                } catch (CommandSyntaxException e) { }
                item.set(DataComponentTypes.CUSTOM_NAME, Text.literal(toName(preset.getLeft())));
                return item;
            });
        ...
        HIDDEN_ENTITIES.forEach((id) -> {
            PRESETS.put(id.getPath()+"_spawn_egg", (preview) -> {
                ...
                var changes = ComponentChanges.builder()
                        ...
                        .add(DataComponentTypes.ENTITY_DATA, TypedEntityData.create(entityType, entityTag))
                        .build();
